...
func (r *RingBuffer[T]) PopFront() T {
	if r.Empty() {
		panic("github.com/refraction-networking/uquic/internal/utils/ringbuffer: pop from an empty queue")
	}
	r.full = false
	t := r.ring[r.headPos]
...
func (r *RingBuffer[T]) PeekFront() T {
	if r.Empty() {
		panic("github.com/refraction-networking/uquic/internal/utils/ringbuffer: peek from an empty queue")
	}
